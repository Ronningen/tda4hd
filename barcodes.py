...
def calc_embed_dist_from_ats(r1, r2, dim = 1, verbose = False, norm = 'quantile', use_max = False, fast = False):
    n = r1.shape[0]
    d = np.zeros((n + r2.shape[0], n + r2.shape[0]))
...
def calc_barcodes_from_ats(d, batch_size = -1, pdist_device = 'cuda:0', dim = 1, is_plot = False):
...
def rtd_from_ats(r1, r2):
    return (h1sum(calc_embed_dist_from_ats(r1, r2)) + h1sum(calc_embed_dist_from_ats(r2, r1)))/2
