...
def get_filtered_mat_list(adj_matrix, thresholds_array, ntokens):
    ...
    for thr in thresholds_array:
        ...
        w_weights = adj_matrix.copy()
        w_weights[filtered_matrix == 0] = 0
        filtered_w_weights.append(w_weights)
        symmetrics.append(w_weights + w_weights.T)
    return filtered_matrices, filtered_w_weights, symmetrics

def adj_m_to_nx_list(adj_matrix, thresholds_array, ntokens, no_mat_output=False, weights=False):
    """
...
#     adj_matrix = adj_matrix[:length,:length]
    filt_mat_list, w_weights, symmetrics = get_filtered_mat_list(adj_matrix, thresholds_array, ntokens)
    nx_graphs_list = []
    nx_graphs_weights = []
    nx_graphs_sym = []
    for i in range(len(filt_mat_list)):
        nx_graphs_list.append(nx.from_numpy_array(np.array(filt_mat_list[i]), \
                                  create_using=nx.MultiDiGraph()))
        nx_graphs_weights.append(nx.from_numpy_array(np.array(w_weights[i]), \
                                  create_using=nx.MultiDiGraph()))
        nx_graphs_sym.append(nx.from_numpy_array(np.array(symmetrics[i]), \
                                  create_using=nx.MultiDiGraph()))
    if no_mat_output:
        return nx_graphs_list, []
    else:
        return nx_graphs_list, nx_graphs_weights, nx_graphs_sym, filt_mat_list

def adj_ms_to_nx_lists(adj_matricies, \
                       thresholds_array, \
                       ntokens_array, \
                       verbose=True, \
                       no_mat_output=False, weights=False):
    """
    ...
    for i in iterable:
        g_list, g_w, g_sym, filt_mat_list = adj_m_to_nx_list(adj_matricies[i],\
                                                 thresholds_array,\
                                                 1000, \
                                                 no_mat_output=no_mat_output, weights=weights)
        graph_lists.append(g_list)
        graph_w_lists.append(g_w)
        graph_sym_lists.append(g_sym)
        filt_mat_lists.append(filt_mat_lists)
    
    return graph_lists, graph_w_lists, graph_sym_lists, filt_mat_lists
...
def count_top_stats(adj_matricies, 
    ...
    for layer_of_interest in range(adj_matricies.shape[1]):
        ...
        for head_of_interest in range(adj_matricies.shape[2]):
            ...
            g_lists, _, _, _ = adj_ms_to_nx_lists(adj_ms,
                                            thresholds_array=thresholds_array,
                                            ntokens_array=[adj_ms.shape[-1] for i in range(adj_ms.shape[0])],
                                            verbose=verbose)
            feat_lists = []
...
def count_edges_btwn(graph_lists, q_len, verbose=False, cap=500):
    e_lists = [] # len == len(pos_w_graph_lists)
    iterable = range(len(graph_lists))
    if verbose:
        iterable = tqdm(range(len(graph_lists)),
                        desc="Calc edges btwn different colors")
    for i in iterable:
        g_list = graph_lists[i]
        e  = []
        for j in range (len(g_list)):
            edges = g_list[j].edges
            cnt = 0
            for u, v, _ in edges:
                if u >= q_len and v < q_len:
                    cnt += 1
            e.append(cnt)
        e_lists.append(e)
    return e_lists

def count_top_stats_extended(adj_matricies,
                             q_matricies,
                             s_matricies,
                             thresholds_array, 
                             ntokens_array, 
    ...
    for layer_of_interest in range(adj_matricies.shape[1]):
        ...
        for head_of_interest in range(adj_matricies.shape[2]):
            sleep(sleep_time)
            adj_ms = adj_matricies[:,layer_of_interest,head_of_interest,:,:]
            adj_q = q_matricies[:,layer_of_interest,head_of_interest,:,:]
            adj_s = s_matricies[:,layer_of_interest,head_of_interest,:,:]
            
            feat_lists = []
            g_lists, g_w_lists, g_sym_lists, _ = adj_ms_to_nx_lists(adj_ms,
                                            thresholds_array=thresholds_array,
                                            ntokens_array=[adj_ms.shape[2] for i in range(adj_ms.shape[2])],
                                            verbose=verbose)
            # on full matrix
            if "f" in stats_to_count:
                feat_lists.append(dim_connected_components(g_lists,
                                                           strong=True,
                                                           verbose=verbose,
                                                           cap=stats_cap))
                ...
                feat_lists.append(b1_lists)
            # on prompt matrix
            if "q" in stats_to_count:
                g_lists_q, g_w_lists_q, g_sym_lists_q, _ = adj_ms_to_nx_lists(adj_q,
                                            thresholds_array=thresholds_array,
                                            ntokens_array=[adj_q.shape[2] for i in range(adj_q.shape[2])],
                                            verbose=verbose)
                feat_lists.append(dim_connected_components(g_lists_q,
                                                           strong=True,
                                                           verbose=verbose,
                                                           cap=stats_cap))
                feat_lists.append(dim_connected_components(g_lists_q,
                                                           ...
                                                           verbose=verbose, 
                                                           cap=stats_cap))
                feat_lists.append(edges_f(g_lists_q, verbose=verbose))
                feat_lists.append(v_degree_f(g_lists_q, verbose=verbose))
                feat_lists.append(dim_simple_cycles(g_lists_q,
                                                    verbose=verbose, 
                                                    cap=stats_cap))
                b0_lists, b1_lists = b0_b1(g_lists_q, verbose=verbose) 
                feat_lists.append(b0_lists)
                feat_lists.append(b1_lists)
            # on sentence matrix
            if "s" in stats_to_count:
                g_lists_s, g_w_lists_s, g_sym_lists_s, _ = adj_ms_to_nx_lists(adj_s,
                                            thresholds_array=thresholds_array,
                                            ntokens_array=[adj_s.shape[2] for i in range(adj_s.shape[2])],
                                            verbose=verbose)
                feat_lists.append(dim_connected_components(g_lists_s,
                                                           strong=True,
                                                           verbose=verbose,
                                                           cap=stats_cap))
                feat_lists.append(dim_connected_components(g_lists_s,
                                                           ...
                                                           verbose=verbose, 
                                                           cap=stats_cap))
                feat_lists.append(edges_f(g_lists_s, verbose=verbose))
                feat_lists.append(v_degree_f(g_lists_s, verbose=verbose))
                feat_lists.append(dim_simple_cycles(g_lists_s,
                                                    verbose=verbose, 
                                                    cap=stats_cap))
                b0_lists, b1_lists = b0_b1(g_lists_s, verbose=verbose) 
                feat_lists.append(b0_lists)
                feat_lists.append(b1_lists)
            # on diff colors
            if "c" in stats_to_count:
                feat_lists.append(count_edges_btwn(g_lists,
                                                   adj_q.shape[2],
                                                   verbose=verbose,
                                                   cap=stats_cap))
            # on symmetric matrix
